...
public extension UIKitSetting where Base : UIImage {
    ...
    ///   - path: 绘制路径
    @discardableResult
    static func linearGradient<StartPoint, EndPoint>(colors: [UIColor], start startPoint: StartPoint, end endPoint: EndPoint, locations: [CGFloat]? = nil, size: CGSize = CGSize(4), path: UIBezierPath? = nil) -> UIImage? where StartPoint : GradientPoint, EndPoint : GradientPoint {
    ...
    ///   - path: 绘制路径
    @discardableResult
    static func radialGradient(colors: [UIColor], locations: [CGFloat]? = nil, size: CGSize = CGSize(4), path: UIBezierPath? = nil) -> UIImage? {
        return UIKitSetting.renderImage(size: size, opaque: false, scale: 0) { ctx in
            if let path = path {
                path.addClip()
            }
