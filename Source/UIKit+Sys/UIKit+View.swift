...
public extension UIKitSetting where Base : UIView {
    ...
    /// 添加子视图
    @discardableResult
    func overlay<T: UIView>(_ subviewClass: T.Type, configure: ((_ view: UIView, _ subview: T)->Void)?) -> Self {
        let subview = subviewClass.init()
        self.base.addSubview(subview)
        configure?(self.base, subview)
        return self
    }
    
    /// 添加子视图
    @discardableResult
    func overlay<T>(_ subview: T?, configure: ((_ view: UIView, _ subview: T)->Void)? = nil) -> Self where T : UIView {
        guard let subview = subview else { return self }
        if let effect = self.base as? UIVisualEffectView {
            effect.contentView.addSubview(subview)
        } else {
            self.base.addSubview(subview)
        }
        configure?(self.base, subview)
...
// MARK: TableView\CollectionView代理设置
